    ...
    <table class="am-table">
      ...
      <c:forEach items="${Units}" var="unit" varStatus="status">
        <tr>
          ...
          <td>${unit.unitsID}</td>
          <td>${unit.recTotal}</td>
          <td><a href="<%=basePath%>userinfo/changeUnits?name=${unit.unitsName}">修改</a>
            <a href="<%=basePath%>userinfo/deleteUnits?name=${unit.unitsName}">删除</a></td>
        </tr>
      </c:forEach>
      </tbody>
    </table>
    ...
    %><li><a href="<%=basePath%>userinfo/resUnitsMain?page=<%=pages-1%>">&laquo;</a></li><%
      ...
      if(pageAmount<=5||pages<3){
        ...
        for (int i = 1; i <= temp; i++) {
          if(i==pages){
    %><li class="am-active"><a href="<%=basePath%>userinfo/resUnitsMain?page=<%=i%>"><%=i%></a></li><%
    }else{
    %><li><a href="<%=basePath%>userinfo/resUnitsMain?page=<%=i%>"><%=i%></a></li><%
        }
      }
