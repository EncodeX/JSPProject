      ...
      <table class="am-table">
        ...
        <c:forEach items="${proposers}" var="proposer" varStatus="status">
          <tr>
            ...
            <td><a href="<%=basePath%>userinfo/changeProposer?name=${proposer.userName}">修改</a>
              <a href="<%=basePath%>userinfo/deleteProposer?name=${proposer.userName}">删除</a></td>
          </tr>
        </c:forEach>
        </tbody>
      </table>
      ...
      %><li><a href="<%=basePath%>userinfo/proposerMain?page=<%=pages-1%>">&laquo;</a></li><%
        ...
        if(pageAmount<=5||pages<3){
          ...
          for (int i = 1; i <= temp; i++) {
            if(i==pages){
      %><li class="am-active"><a href="<%=basePath%>userinfo/proposerMain?page=<%=i%>"><%=i%></a></li><%
      }else{
      %><li><a href="<%=basePath%>userinfo/proposerMain?page=<%=i%>"><%=i%></a></li><%
          }
        }
